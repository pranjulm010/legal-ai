...
const FIELD_LABELS: Record<keyof Omit<DraftFields, "default_region">, string> = {
  name: "Firm name",
...
export default function WorkspacePanel() {
  const { user } = useAuth();
  const canEdit = hasPermission(user?.role, "manage_team");
    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="rounded-xl border border-[#c9a96e]/12 bg-[#0f0c08] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          {(Object.keys(FIELD_LABELS) as (keyof typeof FIELD_LABELS)[]).map((fieldKey) => (
          ...
          <div className="flex flex-col gap-1">
            ...
            {!canEdit ? (
              <p className="text-sm text-[#e0d2ba]">
                {REGIONS.find((r) => r.value === draft.default_region)?.label || draft.default_region}
              </p>
            ) : (
              <select
                value={draft.default_region}
                onChange={(event) => setField("default_region", event.target.value)}
