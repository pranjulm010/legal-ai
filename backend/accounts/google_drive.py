...
def google_drive_oauth_callback(request):
    ...
    if error:
        return HttpResponseRedirect(f"{settings.FRONTEND_URL}/settings?tab=data-connectors&drive_error={error}")
    ...
    return HttpResponseRedirect(f"{settings.FRONTEND_URL}/settings?tab=data-connectors&drive_connected=1")
